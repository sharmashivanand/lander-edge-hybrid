...
/**
 *
 * Plugin Name: WP SILO Menus
 * Plugin URI: https://www.binaryturf.com/wp-silo-menus
 * Description: Gives you an extended control over navigation menus on your WordPress powered website.
 * Version: 1.0
 * Author: Shivanand Sharma
 * Author URI: https://www.binaryturf.com/
 * License: GPL-2.0+
 * License URI: http://www.opensource.org/licenses/gpl-license.php
 * Text Domain: wp-silo-menus
 *
 */
...
define( 'WPSM_SETTINGS_FIELD','wp-silo-menus' );
define( 'WPSM_PLUGIN_NAME', 'WP SILO Menus' );
define( 'WPSM_PLUGIN_URL', plugin_dir_url(__FILE__) );
define( 'WPSM_PLUGIN_DIR', plugin_dir_path(__FILE__) );

define( 'WPSM_SETTINGS_VER', '1.0' );
...
add_filter( 'plugin_action_links_' . plugin_basename(__FILE__), 'wpsm_add_action_links' );

function wpsm_add_action_links ( $links ) {
	
	$link = array(
		'<a href="'. admin_url( 'admin.php?page=' . WPSM_SETTINGS_FIELD ) .'">Settings</a>',
		'<a href="https://www.binaryturf.com/forum/wp-silo-menus">Support</a>'
	);
...
function lander_add_menu_settings_box() {
	foreach ( (array) get_post_types( array(
		...
		//if ( post_type_supports( $type, 'lander-custom-menu-locations' ) ) {
			add_meta_box( 'landermenu', sprintf( __( '%s Custom Menu Locations', 'wp-silo-menus' ), 'wp-silo-menus' ), 'lander_menu_meta_box', $type, 'side', 'default' );
		//}
	}
}
...
?>
	<h3><?php
	echo esc_html( $tax->labels->singular_name ) . ' ' . __( 'Custom Menu Settings', 'wp-silo-menus' );
	?></h3>
	<?php
...
?>
	...
	_e( 'Theme Location', 'wp-silo-menus' );
	?></th>
			<th scope="col" class="manage-column column-menus"><?php
	_e( 'Assigned Menu', 'wp-silo-menus' );
	?></th>
		</tr>
		</thead>
		<?php
...
function lander_get_menus( $args ) {
	...
	if ( is_single() || is_page() || is_home() || wsm_is_woo_shop() ) {
		if ( is_home() ) {
